...
This skill runs under this repo's `.github/copilot-instructions.md`. Findings are reported for human review only — no patch is ever auto-applied, no commit/push happens as part of a scan, and no GitHub issue is filed unless explicitly asked. English only in all output.
